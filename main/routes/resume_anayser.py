...
resume_anayser_bp = Blueprint('resume_anayser', __name__, url_prefix='/resume_anayser')

@resume_anayser_bp.route('/ping')
def ping():
    event_logger.info("resume_anayser ping endpoint called")
    return jsonify({'success': True, 'message': 'pong'})

@resume_anayser_bp.route('/')
def upload_form():
    event_logger.info("Upload form page accessed")
    return render_template('index.html')

@resume_anayser_bp.route('/upload', methods=['POST'])
def upload_resume():
    try:
